...
def plotROC(pred_strengths, class_labels):
    ...
    plt.title('ROC curve for AdaBoost detection system')
    ax.axis([0, 1, 0, 1])
    plt.show()
...
if __name__ == '__main__':
    ...
    for i in range(len(data)):
        if (data[i,0]>=21 and data[i,0]<=27):
            label.append(1.0)
        else:
            label.append(-1.0)
        i=i+1
    ...
    classifier_array, agg_class_est = Adaboost(train_x, train_y,1000)
    ...
    print("accuracy:%.2f" % (1 - err_num / float(1000)))

    #plotROC(agg_class_est.T, train_y)
